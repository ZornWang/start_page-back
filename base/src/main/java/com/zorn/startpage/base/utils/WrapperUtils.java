...
public class WrapperUtils {
    ...
    public static <T,M> QueryWrapper<T> getInQueryWrapper(String key, List<M> value) {
        QueryWrapper<T> queryWrapper = new QueryWrapper<>();
    ...
    /**
     * 通过VO类提取查询条件（排除size和current字段）
     * @param object
     * @param <T>
     * @param <G>
     * @return
     */
    public static <T,G> QueryWrapper<T> queryWrapperByQueryVONoPage(G object) {
        QueryWrapper<T> queryWrapper=new QueryWrapper<>();
        if(object!=null){
            Class<?> clz = object.getClass();
            Field[] fields = clz.getDeclaredFields();
            for (Field field:fields){
                if(field.getName().equals("size")||field.getName().equals("current")){
                    continue;
                }
                if(field.getName().equals("serialVersionUID")){
                    continue;
